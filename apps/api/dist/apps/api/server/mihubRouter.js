...
import { agentTasks, agentBrain, agentMessages, dataBag } from "../drizzle/schema";
...
export const mihubRouter = router({
    ...
    getTasks: publicProcedure
        ...
        if (input.agentAssigned) {
            query = query.where(eq(agentTasks.agentAssigned, input.agentAssigned));
        }
        if (input.status) {
            query = query.where(eq(agentTasks.status, input.status));
        }
    ...
    getBrainMemory: publicProcedure
        ...
        if (input.memoryType) {
            query = query.where(and(eq(agentBrain.agent, input.agent), eq(agentBrain.memoryType, input.memoryType)));
        }
        if (input.key) {
            query = query.where(and(eq(agentBrain.agent, input.agent), eq(agentBrain.key, input.key)));
        }
