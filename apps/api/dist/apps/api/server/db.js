...
export async function getOverviewStats() {
    ...
    const [yesterdayTransactions] = await db
        .select({ count: count() })
        .from(schema.transactions)
        .where(and(gte(schema.transactions.createdAt, yesterday), lt(schema.transactions.createdAt, today)));
    ...
    const [usersPrevWeek] = await db
        .select({ count: count() })
        .from(schema.users)
        .where(and(gte(schema.users.createdAt, fourteenDaysAgo), lt(schema.users.createdAt, sevenDaysAgo)));
    ...
    const [bioProducts] = await db
        ...
        .where(or(eq(schema.products.certifications, 'BIO'), eq(schema.products.certifications, 'KM0')));
...
export async function getReimbursements(status) {
    const db = await getDb();
    if (!db)
        return [];
    if (status) {
...
export async function getMioAgentLogById(id) {
    const db = await getDb();
    if (!db)
        return null;
    try {
...
export async function testDatabaseConnection() {
    ...
    `);
        ...
        if (tableExists) {
            ...
            const countResult = await db.execute("SELECT COUNT(*) as count FROM mio_agent_logs");
