...
export const guardianRouter = router({
    ...
    getEndpointById: publicProcedure
        ...
        for (const service of apiIndex.services) {
            const endpoint = service.endpoints.find((e) => e.id === input.endpointId);
            if (endpoint) {
                return {
                    ...endpoint,
                    service: {
    ...
    getAgentPermissions: publicProcedure
        ...
        const agent = permissions.agents.find((a) => a.id === input.agentId);
        if (!agent) {
            return {
    ...
    checkPermission: publicProcedure
        ...
        const rule = agent.rules.find((r) => r.endpoint_id === input.endpointId);
        if (!rule) {
            const logEntry = {
                timestamp: new Date().toISOString(),
                agent: input.agentId,
    ...
    getLogs: publicProcedure
        ...
        try {
            ...
            // Filtra per agent
            if (input.agent) {
                logs = logs.filter((log) => log.agent === input.agent);
            }
            // Filtra per status
            if (input.status) {
    ...
    getDebugLogs: publicProcedure
        ...
        try {
            ...
            logs = logs.filter((log) => log.status === "denied" || log.status === "error");
            // Ordina per timestamp decrescente e limita
            logs = logs.reverse().slice(0, input.limit);
            return logs;
        }
