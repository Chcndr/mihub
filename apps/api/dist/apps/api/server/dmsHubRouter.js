 ...
 */
async function logAction(action, entityType, entityId, userEmail, oldValue = null, newValue = null, ipAddress = null) {
 ...
 */
function convertSlotEditorV3Format(data) {
    ...
    if (data.container && data.container.length > 0) {
        const lats = data.container.map((p) => p[0]);
        const lngs = data.container.map((p) => p[1]);
        result.center = {
    ...
    if (data.stalls_geojson?.features) {
        result.stalls = data.stalls_geojson.features.map((f) => {
    ...
    if (data.markers_geojson?.features) {
        result.customMarkers = data.markers_geojson.features.map((f) => {
            const props = f.properties || {};
            const coords = f.geometry.coordinates;
            return {
    ...
    if (data.areas_geojson?.features) {
        result.customAreas = data.areas_geojson.features.map((f) => {
            const props = f.properties || {};
            return {
...
export const dmsHubRouter = router({
    ...
    markets: router({
        ...
        importFromSlotEditor: publicProcedure
            ...
            // Log operazione
            await logAction("IMPORT_SLOT_EDITOR", "market", Number(marketId), null, // TODO: prendere da ctx.user quando disponibile
            null, {
        ...
        importAuto: publicProcedure
            ...
            await logAction("AUTO_IMPORT_SLOT_EDITOR", "market", Number(marketId), null, null, {
                marketName,
        ...
        list: publicProcedure.query(async () => {
            ...
            const marketsWithStats = await Promise.all(markets.map(async (market) => {
                const [stallsCount] = await db.select({ count: sql `count(*)` })
                    .from(schema.stalls)
                    .where(eq(schema.stalls.marketId, market.id));
                const [occupiedCount] = await db.select({ count: sql `count(*)` })
                    .from(schema.stalls)
                    .where(and(eq(schema.stalls.marketId, market.id), eq(schema.stalls.status, "occupied")));
                return {
                    ...market,
    ...
    stalls: router({
        ...
        updateStatus: publicProcedure
            ...
            await db.update(schema.stalls)
                ...
                .where(eq(schema.stalls.id, input.stallId));
            // Log operazione
            await logAction("UPDATE_STALL_STATUS", "stall", input.stallId, null, { status: oldStall?.status }, { status: input.status });
            return { success: true };
        }),
    ...
    vendors: router({
        ...
        create: publicProcedure
            ...
            // Log operazione
            await logAction("CREATE_VENDOR", "vendor", vendorId, null, null, { ...input, status: "active" });
        ...
        update: publicProcedure
            ...
            await db.update(schema.vendors)
                ...
                .where(eq(schema.vendors.id, input.vendorId));
            // Log operazione
            await logAction("UPDATE_VENDOR", "vendor", input.vendorId, null, oldVendor, input.data);
            return { success: true };
        }),
        ...
        getFullDetails: publicProcedure
            ...
            const presences = await db.select().from(schema.vendorPresences)
                .where(and(eq(schema.vendorPresences.vendorId, input.vendorId), sql `${schema.vendorPresences.checkinTime} >= ${thirtyDaysAgo}`))
    ...
    bookings: router({
        ...
        create: publicProcedure
            ...
            // Log operazione
            await logAction("CREATE_BOOKING", "booking", bookingId, null, null, { stallId: input.stallId, userId: input.userId, vendorId: input.vendorId, expiresAt });
            return {
                success: true,
                bookingId,
                expiresAt,
        ...
        confirmCheckin: publicProcedure
            ...
            // Log operazione
            await logAction("CONFIRM_CHECKIN", "booking", input.bookingId, null, { status: "pending" }, { status: "confirmed", vendorId: input.vendorId });
            return { success: true };
        }),
        ...
        cancel: publicProcedure
            ...
            await db.update(schema.stalls)
                ...
                .where(eq(schema.stalls.id, booking.stallId));
            // Log operazione
            await logAction("CANCEL_BOOKING", "booking", input.bookingId, null, { status: booking.status }, { status: "cancelled" });
            return { success: true };
        }),
    }),
    // ============================================
    ...
    presences: router({
        ...
        checkout: publicProcedure
            ...
            // Log operazione
            await logAction("CHECKOUT_VENDOR", "presence", input.presenceId, null, { checkoutTime: null }, { checkoutTime: now, duration });
        ...
        getTodayByMarket: publicProcedure
            ...
            const presences = await db.select({
                ...
                .where(and(eq(schema.stalls.marketId, input.marketId), sql `${schema.vendorPresences.checkinTime} >= ${today}`))
                .orderBy(desc(schema.vendorPresences.checkinTime));
            return presences;
        }),
    }),
    ...
    inspections: router({
        ...
        create: publicProcedure
            ...
            // Log operazione
            await logAction("CREATE_INSPECTION", "inspection", inspectionId, input.inspectorName, null, { vendorId: input.vendorId, result: input.result, type: input.type });
    ...
    violations: router({
        ...
        create: publicProcedure
            ...
            // Log operazione
            await logAction("CREATE_VIOLATION", "violation", violationId, null, null, { vendorId: input.vendorId, violationType: input.violationType, fineAmount: input.fineAmount });
